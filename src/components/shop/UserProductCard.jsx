...
const UserProductCard = ({ product }) => {
  const { setProductDetails, setOpenProductDialog } = useContext(MyAppContext);

  const handleClickButton = (getProduct) => {
  ...
  return (
    <div className="py-2 px-1 shadow-green-950/20 shadow-xl rounded-2xl min-h-auto flex flex-col items-center gap-3">
      ...
      <div className="flex items-center justify-between w-full">
        <button
          className="bg-green-50 py-1 text-green-900 hover:text-green-950 px-3 rounded-3xl font-semibold lg:text-lg text-sm hover:bg-green-100 cursor-pointer"
